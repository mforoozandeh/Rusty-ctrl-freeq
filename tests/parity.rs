...
//!
//! The fixtures in `tests/fixtures` were generated by `tools/python_reference/generate.py` from the Python
//! ctrl-freeq 0.3.0, each configuration at a fixed parameter vector, and are kept as that version computed them.
//! Every quantity the Rust setup and objective produce is compared against them - basis matrices, drift
//! Hamiltonians, initial states, targets, modulation, the cost and its gradient - except the comparisons
//! [`Departures`] names, which that version computed differently.
//!
//! `DEVIATIONS.md` lists what still differs from the current Python package, 0.4.0.  None of these departures is on
//! that list, so regenerating the fixtures from 0.4.0 should let their comparisons come back.
...
/// Where a fixture's configuration meets something the 0.3.0 fixtures computed differently.
struct Departures {
    ...
    transmon: bool,
    /// A carrier offset: waveform samples at the middle of each step, not on the fixtures' slightly wider grid.
    carrier: bool,
    /// Relaxation: the dissipator's exact channel in Strang splitting, where the fixtures took an explicit Euler
    /// step.  On the dissipative problem that scheme is 6.4e-4 out in fidelity - 0.298505 against 0.299143 for the
...
fn setup_and_objective_match_python() {
    ...
    for (name, fx) in fixtures() {
        ...
        // Cost and gradient.  The fixtures' Liouville fidelity goes through two eigendecompositions of
        // rank-deficient matrices, which costs it a few digits; Rust's Re Tr(ρσ) is exact for the pure targets.
